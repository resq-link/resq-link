    ...
    try {
      ...
      if (Constants.appOwnership === "expo") {
        setState((current) => ({
          ...current,
          phase: "ringing",
          error: "Expo Go can update the call request, but real Agora audio requires an Expo development build.",
        }));
        return;
      }
      ...
      const message = error?.message || "Voice call failed. The incident remains active.";
      setState((current) => ({ ...current, phase: "failed", error: message }));
      onError?.(error);
      await cleanup();
      throw error;
