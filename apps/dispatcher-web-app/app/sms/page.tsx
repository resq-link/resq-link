...
import { CheckCircle2, ClipboardPlus, MessageSquareMore, Plus, Send, Settings2, ShieldAlert, Trash2, XCircle } from "lucide-react";
import ProtectedRoute from "@/components/ProtectedRoute";
import { useAuth } from "@/contexts/AuthContext";
import { createSmsQuickReply, defaultSmsQuickReplies, deleteSmsQuickReply, subscribeToSmsIntakes, subscribeToSmsMessages, subscribeToSmsQuickReplies, type SmsIntake, type SmsIntakeStatus, type SmsMessage, type SmsQuickReply } from "@packages/firebase";
...
function SmsWorkspace() {
  ...
  const incidentHref = useMemo(() => !selected ? "/intake" : `/intake?${new URLSearchParams({ source: "sms", callerContact: selected.phoneNumber, description: selected.latestMessage, smsThreadId: selected.threadId })}`, [selected]);

  async function callFunction(path: "sendSms" | "updateSmsIntake", payload: Record<string, string>) {
    if (!user || !functionsBaseUrl) throw new Error("SMS functions are not configured.");
    const token = await user.getIdToken();
    const response = await fetch(`${functionsBaseUrl}/${path}`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify(payload) });
    if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "Unable to complete the SMS action.");
  }
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-5 p-4 sm:p-6 lg:p-8">
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary-400">Operations / civilian communications</p><h1 className="mt-1 text-2xl font-semibold text-slate-100">SMS Intake</h1><p className="mt-1 text-sm text-slate-400">Triage civilian reports, gather essentials, and escalate verified emergencies to incident intake.</p></div><div className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-950/70 px-3 py-2 text-xs text-slate-400"><span className="h-2 w-2 rounded-full bg-emerald-400" />Gateway connected</div></header>
    {!functionsBaseUrl && <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-sm text-amber-100">SMS actions need <code>NEXT_PUBLIC_SMS_FUNCTIONS_BASE_URL</code> in the dispatcher deployment.</div>}
    <div className="flex justify-end"><button onClick={() => setIsPromptManagerOpen((open) => !open)} className="inline-flex items-center gap-2 rounded-lg border border-slate-700 px-3 py-2 text-sm font-medium text-slate-300 hover:border-primary-500/70 hover:text-primary-200"><Settings2 size={16} />Manage SMS prompts</button></div>
