...
export function PriorityAlertProvider({ children }: { children: ReactNode }) {
  const { user } = useAuth()
  const { emergencyReports, emergencyReportsMeta, emergencyReportsLoading } = useDispatcherData()
  ...
  useEffect(() => {
    if (!user?.uid || emergencyReportsLoading) return
    processSnapshotRef.current(emergencyReports, emergencyReportsMeta ?? undefined)
  }, [user?.uid, emergencyReports, emergencyReportsMeta, emergencyReportsLoading])
