...
export default function TeamsPage() {
  const { user } = useAuth()
  const { teams, isLoading: loadingTeams } = useOperationalTeams()
  ...
  const defaultTeamCodes = useMemo(
    () => new Set(['whiskey', 'x-ray', 'yankee', 'zulu']),
    []
  )
  ...
  const handleCreateTeam = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setPageMessage(null)
    try {
      const code = teamForm.code.trim().toLowerCase()
      if (defaultTeamCodes.has(code)) {
        throw new Error('Whiskey, X-ray, Yankee, and Zulu are system teams and cannot be recreated.')
      }
      const duplicate = teams.some((team) => team.code.trim().toLowerCase() === code)
      if (duplicate) {
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 md:p-5">
            ...
            <div className="mt-6 border-t border-slate-800 pt-4">
              ...
              <div className="space-y-2">
                {teams.length === 0 ? <div className="rounded-xl border border-dashed border-slate-800 bg-slate-950/40 px-4 py-6 text-sm text-slate-500">No teams yet.</div> : teams.map((team, index) => {
                  const assignedCount = members.filter((member) => member.teamLabel === team.label || member.teamCode === team.code).length
                  return (
                    <div key={teamReactKey(team, index)} className="flex items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-950/50 px-4 py-3">
                      <div>
            ...
            <div className="mb-4 flex flex-col gap-3">
              ...
              <div className="grid gap-2 lg:grid-cols-[1.5fr_0.9fr_0.9fr]">
                ...
                <select value={teamFilter} onChange={(event) => setTeamFilter(event.target.value)} className="h-10 rounded-lg border border-slate-800 bg-slate-950 px-3 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-500">
                  ...
                  {teams.map((team, index) => <option key={teamReactKey(team, index)} value={team.label}>{team.label}</option>)}
                </select>
