...
import TeamListFilterSelect from "@/components/operational/TeamListFilterSelect";
...
function HistoryContent() {
  ...
  const { incidents, emergencyReports } = useDispatcherData();
  const { listTeamFilter, incidentMatchesFilter } = useOperationalTeams();
  const recentIncidents = incidents;
  const [resolvedReports, setResolvedReports] = useState<EmergencyReport[]>([]);
  const appEmergencyReports = useMemo(() => {
    const merged = new Map<string, EmergencyReport>();
    emergencyReports.filter(isResolvedEmergencyReport).forEach((report) => {
  ...
  }, [emergencyReports, resolvedReports]);
  const [selectedQueueItem, setSelectedQueueItem] = useState<IntakeQueueItem | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  useEffect(() => {
    return subscribeToEmergencyReports(
      (reports) => {
        setResolvedReports(reports.filter(isResolvedEmergencyReport));
      },
  ...
  const filteredQueueItems = useMemo(() => {
    ...
    if (listTeamFilter !== "all") {
      items = items.filter(
        (val) => val.rawIncident && incidentMatchesFilter(val.rawIncident)
      );
    }
    ...
    return items;
  }, [resolvedQueueItems, searchQuery, listTeamFilter, incidentMatchesFilter]);
          ...
          <div className="px-4 py-2.5 border-b border-slate-800 bg-slate-900/40">
            <div className="flex flex-nowrap items-center gap-3">
              <div className="flex min-w-0 shrink-0 items-center gap-2.5">
                <HistoryIcon className="w-5 h-5 shrink-0 text-emerald-400" />
                <h2 className="text-sm font-black uppercase tracking-widest text-slate-100 whitespace-nowrap">
                  Resolution Archive
                </h2>
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 whitespace-nowrap">
                  {filteredQueueItems.length} ARCHIVED
                </span>
              </div>

              <div className="ml-auto flex min-w-0 items-center gap-2">
                <TeamListFilterSelect className="shrink-0" />
                <div className="relative w-[240px] max-w-full min-w-[140px]">
