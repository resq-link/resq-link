...
const MapComponent = dynamic(() => import('@/components/MapComponent'), {
  ssr: false,
  loading: () => <InlineLoader label="Initializing response map..." />,
})
...
export default function MapPage() {
  const { emergencyReports, dispatcherLocations } = useDispatcherData()
  const incidents = useMemo(
    () => emergencyReports.map(convertToMapIncident),
    [emergencyReports]
  )
  ...
  const getCurrentLocation = (highAccuracy = true) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      },
      {
        ...
        timeout: highAccuracy ? 10000 : 4000,
