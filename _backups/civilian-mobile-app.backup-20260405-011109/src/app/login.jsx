...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Load thin Firebase entry only when logging in (avoids pulling full package graph at route load).
      let signInCivilian;
      try {
        const m = await import("@packages/firebase/civilian-auth");
        signInCivilian = m.signInCivilian;
      } catch (loadErr) {
        console.error("Firebase import error:", loadErr);
        throw new Error(
          loadErr?.message ||
            "Failed to load Firebase. Check .env / app.json and that @packages/firebase is built (npm run build in packages/firebase)."
        );
      }
      if (typeof signInCivilian !== "function") {
        throw new Error("Firebase package did not export signInCivilian.");
      }
